...
- **Multi-file Server Event Import**: Import multiple dedicated-server event files in a single pick; each file becomes a separate race
...
- **Points & Penalties**: Set a custom points system (comma-separated, with F1/DTM presets); apply points penalties per driver per championship (deducted from the season total, shown as `-5p`) or seconds penalties per driver per race (added to race time, re-sorts positions, shown as `+20s`) — all reflected on-screen and in exported HTML
- **Export Options**: Download a single championship HTML, a full index page, or the JSON database
- **Website Archive Export**: Export all championships as ZIP/7z with `index.html` and championship HTML files in the same archive root folder
- **Local Storage**: Saved automatically in persistent storage for instant access, including custom points systems and penalties
