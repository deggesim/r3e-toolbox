...
- **Edit Mode**: Toggle Edit on a championship to set a custom points system (comma-separated, with F1/DTM presets), add time penalties (seconds; re-sorts positions) or points penalties (deducted from race score) per driver per race — all reflected on-screen and in exported HTML
