...
**No backend required** - runs entirely in your browser with localStorage persistence for caching. Electron mode adds native file dialogs and auto-detection of game files.
...
- **Local Storage**: Saved automatically in `localStorage` for instant access
...
│   ├── championshipStore.ts      # Saved championships in localStorage
